...
**Applying fixes:** Before modifying any file, READ IT COMPLETELY to understand the full context. Fixes applied without understanding surrounding code cause regressions. After applying fixes, re-read each modified file completely to verify correctness.
...
user: |
  TODO_FILE: reports/code-auditor/${TS}-TODO-aimaestro-server-changes.md
  ASSIGNED_TODOS: TODO-AMS1, TODO-AMS2, TODO-AMS3
  FILES: services/governance-service.ts, lib/agent-registry.ts
  CHECKPOINT_PATH: reports/code-auditor/${PIPELINE_TS}-caa-checkpoint-P1-AMS-batch1.json
  REPORT_PATH: reports/code-auditor/${TS}-caa-fixes-done-P1-AMS-batch1.md
...
assistant: |
  ...
  Returns: "[DONE] fix-AMS-batch1 - 3/3 TODOs applied, 0 skipped. Report: reports/code-auditor/${TS}-caa-fixes-done-P1-AMS-batch1.md"
</example>

<example>
Context: Orchestrator spawns this agent to apply TODOs but one has a context mismatch.
user: |
  TODO_FILE: reports/code-auditor/${TS}-TODO-AMCOS-changes.md
  ...
  CHECKPOINT_PATH: reports/code-auditor/${PIPELINE_TS}-caa-checkpoint-P1-AMCOS-batch2.json
  REPORT_PATH: reports/code-auditor/${TS}-caa-fixes-done-P1-AMCOS-batch2.md

  Apply the assigned TODOs to the source files. Make minimal changes only.
assistant: |
  Reads TODO file. Applies fixes one by one with backup/restore for syntax errors. Re-reads each file after fixing.
  Returns: "[DONE] fix-AMCOS-batch2 - 2/3 TODOs applied, 1 skipped. Report: reports/code-auditor/${TS}-caa-fixes-done-P1-AMCOS-batch2.md"
</example>
