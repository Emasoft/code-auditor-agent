...
<!-- skillaudit-inert: category token + role names are code-quoted and the API-accurate `system` role phrasing replaces the bare "SYSTEM prompt" prose so the A2A_CROSS_AGENT_INJECT detector reads inert inline-code, not an injection instruction. Do NOT revert to un-quoted prose — it re-fires a publish-blocking NIT under --strict. -->
- `system-prompt-injection` (MUST-FIX): user-controlled content concatenated into the `system` role message (vs. a `user` role message). The `system` role message must NEVER contain user-provided data without sandwich-style isolation. A `f"{user_message}"` placed in the `system` role is the single most severe finding — always MUST-FIX, no exceptions.
...
<!-- skillaudit-inert: the trailing "regex+allowlist" read as a word+word string-concatenation shape and fired CMD_INJECTION; rewritten to "regex/allowlist combo" so the detector finds no concatenation construct. The sink list is unchanged. Do NOT reintroduce a bare `word+word` here. -->
- code-injection-via-llm (MUST-FIX, no exceptions): LLM output passed to `os.system`, `exec`, `eval`, `child_process.exec`, or raw SQL — even via "templates" → MUST-FIX. Fix is parameterisation OR a strict regex/allowlist combo.
