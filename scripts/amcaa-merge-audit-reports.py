...
# Follows the same pattern as amcaa-merge-reports-v2.sh but for AMCAA audit pipeline.
...
SHOULD_FIX_RE = re.compile(
    r"^#{1,3}\s*(SHOULD.FIX|PARTIALLY IMPLEMENTED|WARNING)", re.IGNORECASE
)
NIT_RE = re.compile(
    r"^#{1,3}\s*(NIT|CONSISTENCY ISSUES|STYLE|SUGGESTION)", re.IGNORECASE
)
CLEAN_RE = re.compile(
    r"^#{1,3}\s*(CLEAN|VERIFIED|COMPLIANT|NO.VIOLATIONS)", re.IGNORECASE
)
...
SKIP_PREFIXES = ("amcaa-fixes-", "amcaa-checkpoint-", "amcaa-fixverify-", "amcaa-manifest-", "TODO-", "amcaa-audit-FINAL-")
...
def main() -> None:
    ...
    if run_id:
        ...
        print(
            f"{YELLOW}No run ID — merging ALL files for pass {pass_number} (legacy mode){NC}"
        )
    ...
    if not reports:
        print(
            f"{RED}Error: No reports matching '{pattern}' found in '{output_dir}'{NC}"
        )
        sys.exit(2)
    ...
    ordered_reports = (
        audit_reports + verify_reports + gapfill_reports + consolidated_reports + other_reports
    )
...
""")
    ...
    if merged_size >= source_total:
        ...
        print(
            f"{GREEN}Integrity check PASSED: merged={merged_size} bytes >= sources={source_total} bytes{NC}"
        )
        ...
        print(
            f"{RED}Integrity check FAILED: merged={merged_size} bytes < sources={source_total} bytes{NC}"
        )
    ...
    print()
    print(
        f"  Report types:    audit={len(audit_reports)}  verify={len(verify_reports)}  gapfill={len(gapfill_reports)}  consolidated={len(consolidated_reports)}  other={len(other_reports)}"
    )
