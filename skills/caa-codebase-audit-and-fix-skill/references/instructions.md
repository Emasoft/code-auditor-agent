...
4. Run Phase 1: spawn `caa-domain-auditor-agent` for each batch (concurrency 20). **Externalizer bulk pre-scan option:** Before spawning agents, optionally call `mcp__plugin_llm-externalizer_llm-externalizer__check_against_specs` with `spec_file_path` (the REFERENCE_STANDARD), `folder_path` (SCOPE_PATH), and `extensions` matching the target language. This checks all source files against the spec in bulk, auto-batching large codebases. Use the resulting violation report to prioritize batches for the agent swarm. Falls back to agent-only discovery if externalizer unavailable.
...
**P6 Fix**: If `llm-externalizer` MCP is available (check via `discover`), use it for **analysis only**: read the Fix Dispatch Ledger, process each report individually. For complex issues, call `mcp__plugin_llm-externalizer_llm-externalizer__code_task` with `input_files_paths` (the source file), `instructions` (project context + issues), and `scan_secrets: true` to get fix guidance. Then apply fixes with Read+Edit tools or spawn `caa-fix-agent`. Do NOT use line numbers — reference function names or quote code. `language` is auto-detected from extension. 115s timeout per call, auto-retries on truncation. Up to 5 analysis calls in parallel. On bad fixes, revert via `git checkout`. Update ledger `fix_status` after each file. Fall back to `caa-fix-agent` if externalizer unavailable. **Fix agent params:** `TODO_FILE`, `ASSIGNED_TODOS`, `FILES`, `CHECKPOINT_PATH`, `REPORT_PATH`. Checkpoint after each fix. Harmonization: preserve existing + add new.
